...
const resetDb = async ({ request }) => {
  writeFileSync(
    dbPath,
    JSON.stringify({ anecdotes: initialAnecdotes }, null, 2),
  );

  await expect(async () => {
    // 1. Use headers for cache-busting instead of URL query parameters
    const response = await request.get("http://127.0.0.1:3001/anecdotes", {
      headers: {
    ...
    expect(
      data.every(
        (a) => a.votes === initialAnecdotes.find((i) => i.id == a.id)?.votes,
      ),
    ).toBe(true);
...
const anecdoteItem = (page, content) =>
  page.getByText(content, { exact: true }).locator("..");

const voteButtonFor = (page, content) =>
  anecdoteItem(page, content).getByRole("button", { name: "vote" });

const deleteButtonFor = (page, content) =>
  anecdoteItem(page, content).getByRole("button", { name: "delete" });
...
test.describe("Anecdotes", () => {
  ...
  test("3 a new anecdote can be created via the form", async ({ page }) => {
    ...
    await expect(
      page.getByText("Freshly baked wisdom", { exact: true }),
    ).toBeVisible();
    await expect(anecdoteItem(page, "Freshly baked wisdom")).toContainText(
      "has 0",
    );
  });

  test("4 the list and the form are separate components sharing the anecdote store", async ({
    page,
  }) => {
    await expect(
      page.getByRole("heading", { name: "Anecdotes" }),
    ).toBeVisible();
    await expect(
  ...
  test("5 voting keeps the anecdote list sorted descending by votes, without mutating state", async ({
    page,
  }) => {
    ...
    let previousTopY = (
      await anecdoteItem(page, previousTopContent).boundingBox()
    ).y;
    ...
    previousTopY = (await anecdoteItem(page, previousTopContent).boundingBox())
      .y;
    risingY = (await anecdoteItem(page, risingContent).boundingBox()).y;
    expect(risingY).toBeLessThan(previousTopY);
  });

  test("6 the filter hides anecdotes that don't match the entered text", async ({
    page,
  }) => {
    await page.getByTestId("filter").fill("Simplicity");

    await expect(
      page.getByText("Simplicity is the ultimate sophistication"),
    ).toBeVisible();
    await expect(page.getByText("Real artists ship code")).not.toBeVisible();
    await expect(
      page.getByText("Untested code is broken code"),
    ).not.toBeVisible();
  });

  test("7 anecdotes are fetched from the JSON Server backend on startup", async ({
    page,
  }) => {
    for (const anecdote of initialAnecdotes) {
      await expect(
        page.getByText(anecdote.content, { exact: true }),
      ).toBeVisible();
    }
    // Sanity check this really is backend data, not the hardcoded default set.
    await expect(
      page.getByText("If it hurts, do it more often"),
    ).not.toBeVisible();
  });

  test("8 a newly created anecdote is persisted to the backend", async ({
    page,
  }) => {
    await page
      .locator('input[name="anecdote"]')
      .fill("Persisted from the form");
    await page.getByRole("button", { name: "create" }).click();
    await expect(
      page.getByText("Persisted from the form", { exact: true }),
    ).toBeVisible();
    ...
    await expect(anecdoteItem(page, "Persisted from the form")).toContainText(
      "has 0",
    );
  });
  ...
  test("10 voting shows a notification that disappears after five seconds", async ({
    page,
  }) => {
  ...
  test("11 an anecdote can only be deleted once it has zero votes", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(zeroVoteContent, { exact: true }),
    ).not.toBeVisible();
  ...
  test("12 state initializes with the exact anecdotes and vote counts from the backend", async ({
    page,
  }) => {
    for (const anecdote of initialAnecdotes) {
      await expect(anecdoteItem(page, anecdote.content)).toContainText(
        `has ${anecdote.votes}`,
      );
    }
  });

  test("13 the anecdote list receives anecdotes already sorted by votes", async ({
    page,
  }) => {
    const boxes = [];
  ...
  test("14 the anecdote list receives a properly filtered, still-sorted set of anecdotes", async ({
    page,
  }) => {
  ...
  test("15 voting repeatedly accumulates the anecdote's vote count in the list", async ({
    page,
  }) => {
    const content = "There are two hard things in computer science";
